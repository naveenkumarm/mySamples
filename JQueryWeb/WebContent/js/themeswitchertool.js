...
---------------------------------------------------------------------*/
jq.fn.themeswitcher = function(settings){
	...
	switcherpane.hover(
		function(){},
		function(){if(switcherpane.is(':visible')){jq(this).spHide();}}
	);

	//show/hide panel functions
	jq.fn.spShow = function(){ jq(this).css({top: button.offset().top + options.buttonHeight + 6, left: button.offset().left}).slideDown(50); button.css(button_active); options.onOpen(); }
	jq.fn.spHide = function(){ jq(this).slideUp(50, function(){options.onClose();}); button.css(button_default); }
	
		
	/* Theme Loading
	---------------------------------------------------------------------*/
	switcherpane.find('a').click(function(){
		updateCSS( jq(this).attr('href') );
		var themeName = jq(this).find('span').text();
		//button.find('.jquery-ui-themeswitcher-title').text( options.buttonPreText + themeName );
		jq.cookie(options.cookieName, themeName);
		options.onSelect();
	...
	function updateCSS(locStr){
		var cssLink = jq('<link href="'+locStr+'" type="text/css" rel="Stylesheet" class="ui-theme" />');
		jq("head").append(cssLink);
		
		
		if( jq("link.ui-theme").size() > 3){
			jq("link.ui-theme:first").remove();
		}	
	}	
	...
	var button_default = {
		...
		fontSize: '14px',
		color: '#FFFFFF',
		//background: '#eee url(http://www.trirand.net/i/buttonbg.png) 50% 50% repeat-x',
	...
	.hover(
		function(){ 
			jq(this).css(button_hover); 
			switcherpane.spShow();
		},
		function(){ 
		 if( !switcherpane.is(':animated') && switcherpane.is(':hidden') ){	jq(this).css(button_default);  }
		}	
	)
	/*.find('.jquery-ui-themeswitcher-icon').css({
		...
		width: '16px',
		height: '16px',
		background: 'url(http://www.trirand.net/i/icon_color_arrow.gif) 50% 50% no-repeat'
	});	*/
	//pane css
	switcherpane.css({
		position: 'absolute',
	...
	.find('li').hover(
		function(){ 
			jq(this).css({
				'borderColor':'#555',
				'background': 'url(http://www.trirand.net/i/menuhoverbg.png) 50% 50% repeat-x',
				cursor: 'pointer'
			}); 
		},
		function(){ 
	...
	jq(this).append(button);
	jq('body').append(switcherpane);
	switcherpane.hide();
	if( jq.cookie(options.cookieName) || options.loadTheme ){
		var themeName = jq.cookie(options.cookieName) || options.loadTheme;
