...
<br></br>
<div id="projectWiseReports"><f:form id="ProjectWiseSearchForm"
	name="ProjectWiseSearchForm">
	...
	<select	style="width: 100%"	data-bind="value: projectReportsView.projectId,optionsValue: 'id', optionsText: 'name',options: projectReportsView.projectArray,optionsCaption:'Select',
					attr: {'title': $root.setTitle(projectReportsView.projectArray(), projectReportsView.projectId)}">
	</select></div>
	</div>
	...
	<div class="float-left" style="margin-left: .5%; width: 35%;"><input
		...
		data-bind="value: projectReportsView.fromDate" /></div>
	...
	<div class="float-left" style="margin-left: .5%; width: 35%;"><input
		...
		data-bind="value: projectReportsView.toDate" /> <span id="dateValid"
	...
	<input type="radio" name="searchType" value="Daily"
		data-bind="checked: projectReportsView.searchType">&nbsp;&nbsp;&nbsp;&nbsp;Daily
	</span></div>
	<div class="float-left" style="margin-left: 0%; width: 15%;"><span>
	<input type="radio" name="searchType" value="Summary"
		data-bind="checked: projectReportsView.searchType">&nbsp;&nbsp;&nbsp;&nbsp;Summary
	</span></div>
...
<div class="normal-grid" id="dailyProjectReports"
	style="width: 90%; display: none; margin-left: 2%;">
<table>
	<thead>
		<tr>
			<th width='12%'>Date</th>
			<th width='13%'>Employee Id</th>
			<th width='17%'>Employee<br />
			Name</th>
			<th width='13%'>Activity</th>
			<th width='25%'>Task</th>
			<th width='26%'>Work Items</th>
	...
	<tbody style="background-color: white !important;"
		...
		<tr>
			<td colspan="7" style="text-align: center;"><span> No
			Data Found </span></td>
		</tr>
	</tbody>
...
<div class="normal-grid" id="projectReportsSummary"
	style="width: 90%; display: none; margin-left: 2%;">
<table>
	<thead>
		<tr>
	...
	<tbody style="background-color: white !important;"
		data-bind="visible: reportsArray().length == 0">
		<tr>
			<td colspan="5" style="text-align: center;"><span> No
			Data Found </span></td>
		</tr>
	</tbody>
	<tbody data-bind="foreach: reportsArray">
...
function AppViewModel() {
	 var self = this;
	 var validateProjectSearch = {}; 
		
		self.reportsArray = ko.observableArray([]);
		self.projectReportsView = {
		...
		self.projectReportsView.projectId.subscribe(function(projectId) {
			YUI().use('node', "io", function(Y) {
				Y.one('#projectReportsSummary').setStyle('display', 'none');
				Y.one('#dailyProjectReports').setStyle('display', 'none');
				self.reportsArray.removeAll();
			});
		});
		validateProjectSearch.errors = ko.validation.group(self.projectReportsView,{deep: true});
		...
		self.exportReports = function() {
			document.location.href = '/ems-web/ems/project/getExcelProjectWise?searchType=project'+self.projectReportsView.searchType();
		}
		...
		self.searchReports = function(){
			if(validateProjectSearch.errors().length == 0)
			{
				if(StartEndCheck())
				{
					YUI().use('io-base', 'node', "json-parse",'array-extras', 'querystring-stringify',"io", function(Y) {
						var projectInfo = 'projectId='+self.projectReportsView.projectId()+'&fromDate='+self.projectReportsView.fromDate()+'&toDate='+self.projectReportsView.toDate();
						projectInfo = projectInfo +'&searchType='+self.projectReportsView.searchType();
			        	//alert(projectInfo);
								...
								on: {
									...
									var reports = message.reportDetails;
										if (message != null) {
											...
											{
												Y.one('#dailyProjectReports').setStyle('display','none');
												Y.one('#projectReportsSummary').setStyle('display','block');
											}
											if(workItemInd == 'Daily')
											{
												Y.one('#projectReportsSummary').setStyle('display','none');
												Y.one('#dailyProjectReports').setStyle('display','block');
											}
						...
						Y.io("/ems-web/ems/project/projectWiseSearch", cfg);
					});
				}
				else
				{
			...
			{
				validateProjectSearch.errors.showAllMessages(true); 
			}
			
		}
		function StartEndCheck()
		   {
		     var StartDate = self.projectReportsView.fromDate();
		     var EndDate = self.projectReportsView.toDate();
...
function initKo() {
	...
	for(var i=0;i<projectList.length;i++){
		self.projectReportsView.projectArray.push(new projectDropDown(projectList[i]['projectId'],projectList[i]['projectName']));
	}
}
//calendar
...
YUI().use('node','node-core','io-base','json-parse','json-stringify','event', function(Y){
	...
	function selectionChangeCallback(date) {
		if (currDateBxId == 'from') {
				self.projectReportsView.fromDate(date);
		}
		if (currDateBxId == 'to') {
				self.projectReportsView.toDate(date);
		}
	}
